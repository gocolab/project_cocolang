### install dockers
```
~% docker-compose --project-name test_cocolang build --no-cache
~% docker-compose --project-name test_cocolang up -d
```
...
~$ docker cp ./initial_data.archive cocolabhub_jupyter_fastapi:/apps/project_cocolabhub/datasets
